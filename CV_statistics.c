		   ...
		   int N, int dist, char *funtype)
{
  ...
  char header[400], out[400];
  sprintf(header, "#1=<d> s.e."); k=3;
  for(ia=0; ia<Na1; ia++){
    sprintf(header, "%s\t%d=<acc>_%.2f s.e.\t%d=<signi> s.e.",
	    header, k, alpha_val[ia], k+2); k+=4;
  }
  if(Na>Na1){
    sprintf(header, "%s\t%d=<acc>_alpha s.e.\t%d=<signi> s.e.",
	    header, k, k+2); k+=4;
  }
  sprintf(header, "%s\t%d=t s.e.\t%d=Sign s.e.\t%d=TIV s.e.\t%d=Outg\t%d=num",
	  header, k, k+2, k+4, k+6, k+7);
		...
		double *Out1)
{
  ...
  for(int ia=0; ia<Na; ia++){
    ...
    sprintf(out, "%s\t%.3f\t%.2f", out, Acc1[ia], Acc2[ia]);
    p= num_signi[ia]/(float)num;
    if(p > 1){
      printf("Error n_significant= %d  n=%d  alpha=%.2f\n",
	     num_signi[ia], num, alpha_val[ia]);
    }
    sprintf(out, "%s\t%.3f\t%.2f", out, p, sqrt(p*(1-p)/num));
  }
  Ave_se(t1, t2, num);
  sprintf(out, "%s\t%.3f\t%.2f", out, *t1, *t2);
  p=*Sign1/(*Out1);
  sprintf(out, "%s\t%.3f\t%.2f", out, p, sqrt(p*(1-p)/(*Out1)));
  p=*TV1/(*Out1+*TV1);
  sprintf(out, "%s\t%.3f\t%.2f", out, p, sqrt(p*(1-p)/(*Out1+*TV1)));
  sprintf(out, "%s\t%.1f\t%d", out, (*Out1)/num, num);
}

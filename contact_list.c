			 ...
			 int IJ_MIN)
{
  ...
  short *n_cont=malloc(N_res*sizeof(short));
...
int Contact_all(struct atom *atom_i, int ni, struct atom *atom_j, int nj)
{
  ...
  for(i=0; i<ni; i++){
    atom2=atom_j;
    for(j=0; j<nj; j++){
      dx=(atom1->r[0]-atom2->r[0]); if(fabs(dx)>CONT_THR) goto new;
      dy=(atom1->r[1]-atom2->r[1]); if(fabs(dy)>CONT_THR) goto new;
      dz=(atom1->r[2]-atom2->r[2]); if(fabs(dz)>CONT_THR) goto new;
	    ...
	    char *ATOM)
{
  ...
  dx=(atom1->r[0]-atom2->r[0]); if(fabs(dx)>CONT_THR) return(0);
  dy=(atom1->r[1]-atom2->r[1]); if(fabs(dy)>CONT_THR) return(0);
  dz=(atom1->r[2]-atom2->r[2]); if(fabs(dz)>CONT_THR) return(0);
  if((dx*dx+dy*dy+dz*dz)<=CONT_THR_2)return(1);
  return(0);
}
